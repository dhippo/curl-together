...
const char *find_question_type(const char *word) {
    static const char* question_types[][5] = {
            ...
            {"description", "description", "déroule", "expliquer", "explique"},
...
int main() {
    ...
    printf("Bonjour je suis StephaneBot : Le chatbot virtuel de Together & Stronger.\n ");
    printf("Je suis là pour répondre à vos questions sur le déroulé des activités,\n ");

    while (1) {
        printf("Comment puis-je vous aider ? (Appuyez sur 'q' pour quitter)\n ");
        fgets(question, sizeof(question), stdin);
        question[strcspn(question, "\n")] = 0; // Supprimer le caractère de nouvelle ligne

        if (strcmp(question, "q") == 0 || strcmp(question, "Q") == 0) {
            break;
        }

        question_treatment(question);

        printf("\n"); // Imprime une ligne vide pour séparer les questions
    }
